 ...
 */
function removeLeadingAndTrailingWhitespaces(param) {
  return param.trim();
}
 ...
 */
function removeLeadingWhitespaces(param) {
 ...
 */
function removeTrailingWhitespaces(param) {
 ...
 */
function removeLastOccurrences(str, pattern) {
  const replacement = '';
  const match =
 ...
 *   sumOfCodes() => 0
 */
function sumOfCodes(value) {
  if (value === null || value === undefined) return 0;
  let charCode = 0;
  for (let char = 0; char < value.length; char += 1) {
    charCode += value.charCodeAt(char);
  }
  return charCode;
}
